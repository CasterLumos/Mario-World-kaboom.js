...
scene("start", ({level, score, big})=>{
    add([ text('Super Mario World', 36), origin('center'), pos(width()/2, height()/2) ])
    add([ text('Recorde:20', 12), origin('center'), pos(width()/2, 20) ])
    add([ text('Press Enter', 12), origin('center'), pos(width()/2, (height()/2)+50) ])
    keyPress('enter', () => {
...
scene("game", ({ level, score, big })=>{
    ...
    const player = add([
        sprite("mario", { animSpeed: 0.08, frame: 0 }),
        solid(),
        body(),
        pos(60,0),
        big(),
...
scene("lose", ({score}) => {
    ...
    keyPress('space', () => {
        go("start", {level: 0, score: 0, big: isBig})
    })
})
